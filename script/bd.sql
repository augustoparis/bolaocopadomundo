DELIMITER ;

CREATE DATABASE  IF NOT EXISTS `BOLAO`;
USE `BOLAO`;

-- --------------------------------------------------------------------------------
-- TABLES
-- --------------------------------------------------------------------------------

CREATE TABLE `USERS` (
  `ID_USER` SMALLINT(6) NOT NULL AUTO_INCREMENT,
  `NAME` VARCHAR(255) DEFAULT NULL,
  `EMAIL` VARCHAR(255) DEFAULT NULL,
  `LOGIN` VARCHAR(10) DEFAULT NULL,
  `PASSWORD` VARCHAR(255) DEFAULT NULL,
  `ASSETS` INT(11) DEFAULT NULL,
  PRIMARY KEY  (`ID_USER`)
) ENGINE=INNODB AUTO_INCREMENT=1 DEFAULT CHARSET=LATIN1;

CREATE TABLE `BET` (
  `ID_BET` SMALLINT(6) NOT NULL AUTO_INCREMENT,
  `ID_USER` SMALLINT(6) UNSIGNED NOT NULL,
  `VALUE` DECIMAL(10,2) DEFAULT NULL,
  `RESULT1` SMALLINT(6) DEFAULT NULL,
  `RESULT2` SMALLINT(6) DEFAULT NULL,
  PRIMARY KEY  (`ID_BET`)
) ENGINE=INNODB AUTO_INCREMENT=1 DEFAULT CHARSET=LATIN1;
  
-- --------------------------------------------------------------------------------
-- STORED PROCEDURES
-- --------------------------------------------------------------------------------

DELIMITER $$

CREATE PROCEDURE `SX_USERS`(
	INOUT PID_USER INT,
	IN PNAME VARCHAR(255), 
	IN PEMAIL VARCHAR(255), 
	IN PLOGIN VARCHAR(10),
	IN PPASSWORD VARCHAR(255), 
	IN PASSETS INT
)
BEGIN
	INSERT INTO USUARIOS VALUES (PID_USER,PNAME,PEMAIL,PLOGIN,PPASSWORD,PASSETS)
		ON DUPLICATE KEY
	UPDATE NAME=PNAME,EMAIL=PEMAIL,LOGIN=PLOGIN,PASSWORD=PPASSWORD,ASSETS=PASSETS;   

	SELECT LAST_INSERT_ID() INTO PID_USER;
END

DELIMITER $$

CREATE PROCEDURE `SX_BET`(
	INOUT PID_BET INT,
	IN PID_USER SMALLINT(6),
	IN PVALUE DECIMAL(10,2), 
	IN PRESULT1 SMALLINT(6), 
	IN PRESULT2 SMALLINT(6)
)
BEGIN
    	INSERT INTO AUTOBKP VALUES (PID_BET,PID_USER,PVALUE,PRESULT1,PRESULT2)
		ON DUPLICATE KEY
    	UPDATE ID_BET=PID_BET,ID_USER=PID_USER,VALUE=PVALUE,RESULT1=PRESULT1,
		RESULT2=PRESULT2;   

	SELECT LAST_INSERT_ID() INTO PID_BET;
END
